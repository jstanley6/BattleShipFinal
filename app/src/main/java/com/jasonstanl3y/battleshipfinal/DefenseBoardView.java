...
//        canvas.drawText("3", textX, textY, paint);
        ...
        for (int y = 0; y < 11; y++) {
            for (int x = 0; x < 11; x++) {
                if (BoardSetup.defendingGrid[x][y].getHit()) {
                    //Draw * in cell
                    drawCell("X", BoardSetup.defendingGrid, x, y, center, textHeight, canvas);
                } else if (BoardSetup.defendingGrid[x][y].getHasShip()) {
                    // Draw a S in the cell
                    drawCell("S", BoardSetup.defendingGrid, x, y, center, textHeight, canvas);

                } else if (BoardSetup.defendingGrid[x][y].getMiss()) {
                    // Draw an - in the cell
                    drawCell("-", BoardSetup.defendingGrid, x, y, center, textHeight, canvas);

                } else if (BoardSetup.defendingGrid[x][y].getWaiting()) {
                    // Draw a W in the cell

                    drawCell("W", BoardSetup.defendingGrid, x, y, center, textHeight, canvas);
